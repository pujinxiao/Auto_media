...
Step 2.5  角色锚定 ─── generate_character_image     # 生成参考图 → 提取 Visual DNA（详见 video-pipeline.md）
...
| 文件 | 职责 |
|------|------|
| `app/services/story_llm.py` | 剧本全流程 prompt（Step 1-3） |
| `app/services/storyboard.py` | 分镜导演 prompt（Step 4） |
| `app/schemas/storyboard.py` | Shot / CameraSetup / VisualElements / AudioReference 数据模型 |
| `app/services/image.py` | 角色设计图 prompt + Visual DNA 提取（Step 2.5） |
...
**法则 3：角色一致性嵌入** — 每个含角色的 Shot，`subject_and_clothing` 必须使用该角色的 **Visual DNA**（固定描述串），禁止 LLM 自由发挥外貌描述。Visual DNA 由 Step 2.5 生成并存储在 `character_images[name]["visual_dna"]` 中。
...
**解决方案**：在 Step 2.5 生成角色参考图后，提取一段固定的英文外貌描述串（Visual DNA），存入 `character_images[name]["visual_dna"]`，后续所有分镜中该角色的 `subject_and_clothing` 字段强制使用此固定描述，不让 LLM 自由发挥。
...
**存储位置**：`story.character_images` JSON 字段
...
**注入方式**：在 `_build_character_section()` 中，将 `visual_dna` 作为强制约束传给分镜 LLM（替代让 LLM 自行描述角色外貌）。具体注入逻辑详见 [video-pipeline.md](video-pipeline.md)。
...
当前所有提示词分散在业务逻辑文件中，与数据库操作、API 调用、JSON 解析等代码混杂，维护成本高：

| 文件 | 行数 | 提示词 | 问题 |
|------|------|--------|------|
| `app/services/story_llm.py` | 462 | ANALYZE / WB / OUTLINE / SCRIPT / REFINE + apply_chat 动态构建 | 提示词模板与 LLM 调用、数据库读写、JSON 解析混在一起 |
| `app/services/storyboard.py` | 179 | SYSTEM_PROMPT + USER_TEMPLATE + `_build_character_section()` | 提示词占文件 60%+，与解析逻辑耦合 |
| `app/services/image.py` | 186 | `_build_character_prompt()` + VISUAL_DNA_PROMPT（待新增） | 提示词散落在图片生成逻辑中 |

**完整调用链路**（重构时必须全部验证）：

```
app/routers/story.py
  ├─ api_analyze_idea()     → story_llm.analyze_idea()      → ANALYZE_PROMPT.format()
  ├─ api_generate_outline() → story_llm.generate_outline()   → OUTLINE_PROMPT.format()
  ├─ api_generate_script()  → story_llm.generate_script()    → SCRIPT_PROMPT.format()
  ├─ api_refine()           → story_llm.refine()             → REFINE_PROMPT.format()
  ├─ api_apply_chat()       → story_llm.apply_chat()         → 动态 f-string 构建
  ├─ api_wb_start()         → story_llm.world_building_start()→ WB_SYSTEM_PROMPT（system role）
  │                                                            + f"种子想法：{idea}..."（user role）
  └─ api_wb_turn()          → story_llm.world_building_turn() → 从 DB 读取 wb_history（含 WB_SYSTEM_PROMPT）

app/routers/pipeline.py + app/services/pipeline_executor.py
  └─ generate_storyboard()  → storyboard.parse_script_to_storyboard()
                               ├─ _build_character_section(character_info)
                               ├─ SYSTEM_PROMPT（system role）
                               └─ USER_TEMPLATE.format()（user role）

app/routers/character.py
  └─ generate_single/all()  → image.generate_character_image()
                               └─ _build_character_prompt(name, role, description)
```

**chat() 函数注意**：`story_llm.chat()` 是纯透传（直接将 user message 发给 LLM），无自定义 prompt 模板，不需要迁移。

**痛点**：
- 修改一个提示词需要在业务文件中翻找，容易误改业务逻辑
- 无法统一审查所有提示词的风格一致性
- 多人协作时提示词变更和业务代码变更混在同一 diff 中
- 新增提示词（如 Visual DNA、Judge Agent）没有明确的归属位置

### 目标结构

将所有提示词提取到独立的 `app/prompts/` 目录，按管线阶段组织：

```
app/prompts/
├── __init__.py              # 统一导出所有 prompt
├── story.py                 # Step 1-3：ANALYZE / WB / OUTLINE / SCRIPT / REFINE / apply_chat 模板
├── storyboard.py            # Step 4：分镜导演 SYSTEM_PROMPT + USER_TEMPLATE
├── character.py             # Step 2.5 / 5：角色设计图 prompt + VISUAL_DNA_PROMPT
└── judge.py                 # Step 4.5：Judge Agent 审查 prompt
```

### 文件职责划分

**`app/prompts/story.py`** — 剧本生成全流程

```python
# 所有剧本相关 prompt 模板，仅含字符串常量和简单的格式化函数
...
def build_apply_chat_prompt(change_type: str, current_item: dict, history_text: str) -> str:
    """根据修改类型动态构建 apply_chat 的 user prompt。"""
    ...
```

**`app/prompts/storyboard.py`** — 分镜导演

```python
SYSTEM_PROMPT = """..."""
USER_TEMPLATE = """..."""

def build_user_prompt(character_section: str, script: str) -> str:
    return USER_TEMPLATE.format(character_section=character_section, script=script)
```

**`app/prompts/character.py`** — 角色视觉

```python
VISUAL_DNA_PROMPT = """..."""

def build_character_prompt(name: str, role: str, description: str) -> str:
    """构建角色设计图 prompt（含 role 自动映射）。"""
    ...

def build_character_section(characters: list, character_images: dict) -> str:
    """构建传给分镜 LLM 的角色参考信息块。"""
    ...
```

**`app/prompts/judge.py`** — 分镜审查

```python
JUDGE_PROMPT = """..."""
```

**`app/prompts/__init__.py`** — 统一导出

```python
from .story import (
    ANALYZE_PROMPT, WB_SYSTEM_PROMPT, OUTLINE_PROMPT,
    SCRIPT_PROMPT, REFINE_PROMPT, build_apply_chat_prompt,
)
from .storyboard import SYSTEM_PROMPT as STORYBOARD_SYSTEM_PROMPT, build_user_prompt
from .character import VISUAL_DNA_PROMPT, build_character_prompt, build_character_section
from .judge import JUDGE_PROMPT
```

### 原业务文件的变化

重构后，业务文件只保留调用逻辑，提示词全部从 `app/prompts` 导入：

```python
# app/services/story_llm.py（重构后）
from app.prompts import ANALYZE_PROMPT, OUTLINE_PROMPT, SCRIPT_PROMPT


async def analyze_idea(idea, genre, tone, db, **kwargs):
    prompt = ANALYZE_PROMPT.format(idea=idea, genre=genre, tone=tone)
    result = await llm.complete(system="", user=prompt)
    return _parse_json(result)
```

```python
# app/services/storyboard.py（重构后）
from app.prompts import STORYBOARD_SYSTEM_PROMPT, build_user_prompt


async def parse_script_to_storyboard(script, provider, model=None, **kwargs):
    user_msg = build_user_prompt(character_section, script)
    raw, usage = await llm.complete_with_usage(system=STORYBOARD_SYSTEM_PROMPT, user=user_msg)
    return _parse_shots(raw), usage
```

### 迁移步骤

1. 创建 `app/prompts/` 目录和 4 个文件
2. 从 `story_llm.py` 提取 5 个常量 + `apply_chat` 模板构建逻辑 → `prompts/story.py`
3. 从 `storyboard.py` 提取 `SYSTEM_PROMPT` + `USER_TEMPLATE` + `_build_character_section()` → `prompts/storyboard.py` + `prompts/character.py`
4. 从 `image.py` 提取 `_build_character_prompt()` → `prompts/character.py`
5. 原文件改为 `from app.prompts import ...`
6. 运行全量测试验证无回归

### 迁移陷阱（必须注意）

**陷阱 1：`world_building_turn()` 不直接引用 `WB_SYSTEM_PROMPT`**

`world_building_turn()` 从数据库读取 `wb_history`（包含首轮注入的 WB_SYSTEM_PROMPT），不会再次引用常量。这意味着：
- 修改 `WB_SYSTEM_PROMPT` 后，**已创建的**世界构建会话不会受影响（历史已固化在 DB 中）
- 只有**新创建**的会话（`world_building_start()`）会使用新版本
- 这不是 bug，但迁移时需确认 `world_building_start()` 正确导入了新位置的常量

**陷阱 2：`world_building_start()` 有内联 user message**
...
这个 f-string 不是独立常量，而是内联在函数中。迁移时需要一并提取到 `prompts/story.py`：
...
**陷阱 3：`apply_chat()` 的提示词依赖运行时 JSON 序列化**

`apply_chat()` 中的 prompt 通过 `json.dumps(current_item, ensure_ascii=False)` 动态嵌入 JSON。迁移时应将模板和 format 函数一起迁移：

```python
# prompts/story.py
APPLY_CHAT_CHARACTER_TEMPLATE = (
    "以下是关于角色「{name}」的修改讨论：\n\n"
    "{history_text}\n\n"
...
def build_apply_chat_prompt(change_type: str, current_item: dict, history_text: str) -> str:
    ...
    if change_type == "character":
        return APPLY_CHAT_CHARACTER_TEMPLATE.format(
            name=current_item.get("name", ""),
            history_text=history_text,
            current_item_json=current_item_json,
        )
    else:
        return APPLY_CHAT_EPISODE_TEMPLATE.format(
...
注意模板中的 `{{` 和 `}}` 是转义的花括号（Python `.format()` 语法），避免与 JSON 花括号冲突。

### 不需要迁移的部分

| 函数 | 原因 |
|------|------|
| `chat()` | 纯透传，无自定义 prompt，直接将用户消息发给 LLM |
| `world_building_turn()` | 从 DB 读取已有 `wb_history`，不引用 prompt 常量 |
| `_make_client()` / `_get_model()` / `_parse_json()` | 工具函数，与 prompt 无关 |
| `MODEL_MAP` | 模型配置，非 prompt |

### 收益

| 维度 | 重构前 | 重构后 |
|------|--------|--------|
| 查找提示词 | 需在 3 个业务文件中搜索 | 在 `app/prompts/` 下一目了然 |
| 修改提示词 | diff 混杂业务代码变更 | diff 只涉及 prompt 文件 |
| 新增提示词 | 不知道放哪个文件 | 按管线阶段放入对应文件 |
| 风格审查 | 需要逐文件检查 | 在一个目录下统一审查 |
| 测试 | 提示词变更可能误触业务逻辑 | prompt 文件无副作用，可独立测试 |
