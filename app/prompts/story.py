...
REWRITE_IDEA_PROMPT = """你是一位资深短剧编剧，正在帮助用户把“故事灵感”改写成更适合后续短剧创作的种子文本。

你的任务是：根据用户指定的修改要求，对当前灵感做“受控改写”。
...
硬性规则：
1. 如用户指定了故事题材，必须尽量贴合该题材；同时必须保留原始灵感的核心，不得把故事改写成另一个题材或另一个主冲突
2. 如当前灵感与指定题材存在轻微冲突，应优先保留原始核心设定，只做有限收束，并在 `guardrail_notice` 中提醒
3. 不得随意替换主角身份、核心关系、故事方向；如用户要求明显偏离，只能做有限调整，并在 `guardrail_notice` 中提醒
4. 改写目标是让灵感更清晰、更有短剧感、更具视觉化和冲突性，但仍然只是“灵感种子”，不是完整世界观或大纲
5. 改写结果保持 1-3 句，语言自然，适合直接进入后续世界观构建
6. 不要输出完整世界观、分集大纲、对白、分镜或设定表
7. 这次只返回一个“短剧感增强版”：优先增强短剧开场吸引力、人物命运转折和冲突张力，但不能跑偏

严格以 JSON 返回：
{{
  "guardrail_notice": "如用户要求存在跑偏风险，在这里提醒；否则返回空字符串",
  "rewritten_idea": "改写后的灵感文本",
  "rewrite_reason": "一句话说明本次如何在不跑偏的前提下增强短剧感"
}}
...
POLISH_VISUAL_STYLE_PROMPT = """你是一位擅长 AI 影像生成的视觉总监，需要把用户输入的自然语言偏好整理成稳定、统一、可直接用于视觉生成的视觉风格描述。
...
任务目标：
把用户描述整理成一段稳定可用的视觉风格提示词，用于统一角色图、场景图和视频画面风格。

硬性规则：
1. 只处理视觉风格，不得改写剧情、人物关系、世界观、台词或分镜
2. 必须优先保留用户核心偏好；如果用户是在当前风格基础上追加修改，也要尽量继承已有风格方向
3. 输出聚焦这些稳定维度：整体参考风格、画面质感、色彩倾向、光线氛围、细节密度
4. 如用户有否定约束，例如“不要太夸张”“霓虹克制一些”，必须保留这类约束
5. 禁止堆砌模型黑话或无效词，例如“8k”“best quality”“大师级”“超高清”
6. 不要写镜头运动、剪辑节奏、剧情场面、角色外貌设定或具体场景剧情
7. 输出保持简洁自然，适合作为统一视觉风格锁，控制在 1 句或 2 个短句内

严格以 JSON 返回：
{{
  "polished_style": "整理后的视觉风格描述"
}}
...
4. 如果用户指定了故事题材，后续 6 轮提问和最终 `world_summary` 都必须优先贴合该题材；如种子想法与题材略有冲突，尽量收束但保留核心设定
5. 世界观核心维度（按优先级）：时代背景、权力结构、主角处境、核心冲突、主要人物（数量/性格/关系）、情感基调
6. 必须问满 6 轮，第 6 轮结束后返回 complete，不得提前结束
7. complete 时的 world_summary 必须包含所有已收集的维度信息和人物设定
...
硬性结构约束：
1. `meta.episodes` 固定为 {episode_count}，不得改成其他数字
2. `outline` 必须且只能包含 {episode_count} 个对象，对应完整的第 1 集到第 {final_episode} 集
3. `outline` 中的 `episode` 必须严格按 {episode_numbers} 连续返回，禁止跳号、重复、乱序
...
7. 输出前先自检：
   - `meta.episodes == {episode_count}`
   - `outline.length == {episode_count}`
   - `outline.map(ep => ep.episode) == [{episode_numbers}]`
...
最终输出时，`outline` 必须完整写出第 1 集到第 {final_episode} 集的全部内容。
...
7. 第 1 集负责建立人物与主冲突，中段持续升级矛盾，最后一集完成核心收束；不要把主要推进都堆在第 1 集
...
硬性要求：
1. `meta.episodes` 固定为 {episode_count}
2. `season_plan.episode_arcs` 必须完整包含第 1 集到第 {final_episode} 集，且 `episode` 严格按 {episode_numbers} 连续返回
...
2. 仍然要保证第 1 集建立人物与主冲突，中段持续升级，最后一集完成核心收束
